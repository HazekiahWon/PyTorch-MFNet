...
def get_hmdb51(data_root='./dataset/HMDB51',
               ...
               seed=torch.distributed.get_rank() if torch.distributed.is_available() and torch.distributed.is_initialized() else 0,
               **kwargs):
...
def get_ucf101(data_root=os.path.join('.','dataset','UCF101'),
               clip_length=8,
               train_interval=2,
               val_interval=2,
               mean=[0.485, 0.456, 0.406],
